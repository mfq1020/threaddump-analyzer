...
import static site.hearen.threaddump.enums.LineTypeEnum.TITLE;
import static site.hearen.threaddump.util.LineParser.parseCallStack;
import static site.hearen.threaddump.util.LineParser.parseLockHeld;
import static site.hearen.threaddump.util.LineParser.parseState;
import static site.hearen.threaddump.util.LineParser.parseTitle;
import static site.hearen.threaddump.util.LineParser.parseWaitingLock;
import static site.hearen.threaddump.util.LineTypeChecker.getLineType;
import static site.hearen.threaddump.util.ThreadUtils.getLast;
...
import site.hearen.threaddump.enums.DumpVersionEnum;
import site.hearen.threaddump.enums.LineTypeEnum;
import site.hearen.threaddump.util.version.LineTypeAbstract;
import site.hearen.threaddump.util.version.LineTypeHotSpot;
import site.hearen.threaddump.util.version.LineTypeOpenJdk;
...
//        log.info("Parsed Thread Entities: \n{}", toJsonString(threadDoList));
    ...
    private static List<String> getCallStack(ThreadDo threadDo) {
        ...
        threadDo.getDetails().stream().forEach(line -> {
            if (getLineType(line, lineTypeAbstract) == LineTypeEnum.STACK_TRACE) {
    ...
    private static List<List<String>> splitUpLinesByCount(List<String> lines, int blockCount) {
        ...
        for (int i = 0; i < blockCount; ++i) {
            while (end < lines.size() && getLineType(lines.get(end), lineTypeAbstract) != TITLE) {
                end++;
            }
    ...
    private static void parseNewLine(List<ThreadDo> threadDoList, String newLine) {
        ...
        LineTypeEnum lineTypeEnum = getLineType(newLine, lineTypeAbstract);
    ...
    private static List<String> removeInvalidFooter(List<String> lines) {
        int i = lines.size() - 1;
        while (i > -1 && getLineType(lines.get(i), lineTypeAbstract) != LineTypeEnum.THREAD_BLOCK_END) {
            i--;
        }
